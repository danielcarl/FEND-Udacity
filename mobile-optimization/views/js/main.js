...
Modified by:
Daniel Carl, Udacity Front-End Nanodegree Student
*/
...
var resizePizzas = function(size) {
  ...
  /*
    ...
    Parameter: size, an integer representing each size of pizza (1: small, 2: medium, 3: large)
    For optimization, I removed the multiple calls to document.querySelectorAll() from within the for loop,
    and instead assigned the call as a single variable (randomPizzas) which is made before the loop. The resizing
    has also been optimzed - rather than performing math calculations within the loop, the new size is instead
    assigned to newWidth via the case assignment on size. The value of size is assigned randomly via the
    randomPizzas function (above). The assigned numbers needed adjustment (a width of 0.25% was WAY too tiny!),
    and the switch statement was missing break lines, which I added.
  */
...
// I Adjusted maximum randomPizzas down to 35
for (var i = 2; i < 35; i++) {
...
/*
  ...
  I moved the call to document.body.scrollTop out of the for loop, since it was calling the same value each time through.
  Instead, it is assigned to a variable (scrollPositionChange) which is used within the loop instead of the repeated call.
*/
function updatePositions() {
  frame++;
  window.performance.mark("mark_start_frame");
  var items = document.querySelectorAll('.mover');
  var scrollPositionChange = document.body.scrollTop / 1250;
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin((scrollPositionChange) + (i % 5));
...
// I Adjusted maximum randomPizzas down to 35, to match the for loop at line 462.
